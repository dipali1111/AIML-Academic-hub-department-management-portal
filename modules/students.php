...
$yearLabels = [1 => 'FY', 2 => 'SY', 3 => 'TY', 4 => 'FINAL YEAR'];
...
<div class="card">
    ...
    <form method="post" class="grid grid-3">
        ...
        <div class="form-group"><label>Year</label><select name="year"><?php foreach ($yearLabels as $k => $v): ?><option value="<?= $k ?>" <?= ($edit['year'] ?? 1) == $k ? 'selected' : '' ?>><?= $v ?></option><?php endforeach; ?></select></div>
...
<div class="card">
    ...
    <div class="table-wrap"><table>
        ...
        <tr>
            ...
            <td><?= e($s['full_name']) ?></td>
            <td><?= e($s['email']) ?></td>
            <td><?= ($yearLabels[$s['year']] ?? $s['year']) ?> / Sem <?= $s['semester'] ?></td>
            <td><?= e($s['division']) ?></td>
            <td><?= $s['cgpa'] ?></td>
            <td>
